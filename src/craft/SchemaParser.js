...
class SchemaParser {
        ...
        for (let i = 0; i < targetPaths.length; i++) {
            ...
            if (!targetTemp) {
                ...
                if (arrTargetTemp.type === "array") {
                    ...
                    arrAfterPath = targetPaths[i] + "." + targetPaths.slice(i + 1).join(".")
                    break;
                }
            }
        }
    ...
    genNormalCallback(root, func, params, basePath, bindPath) {
        ...
        params.forEach((param) => {
            const [getValuesFunc, callbackPaths] = this.getTrueParam(root, basePath, param);
    ...
    parseProxy(root, parsedData, path = "", parent = null, property = "") {
        ...
        if (type === "object") {
            ...
            for (let i = 0; i < parsedData.value.length; i++) {
                ...
                this.parseProxy(root, parsedData.value[i], childTotalPath, parsedData, i);
            }
        } else if (BasicType.includes(type)) {
            const totalPath = `${path}.${parsedData.title}`;
            if (root.callbacks.hasOwnProperty(totalPath)) {
                // console.log(totalPath,parent,property);
                const [bindParent, bindProperty] = this.findParent(root, root.callbacks[totalPath].bindPath);
    ...
    findParent(root, path = "") {
        ...
        return [this.find(root, root.rootData, paths.slice(0, len - 1).join(".")), paths[len - 1]];
    }
    //DONE: 根据path获取对应的对象
    /**
     * 路径算法：
